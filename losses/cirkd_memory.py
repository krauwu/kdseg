...
class StudentSegContrast(nn.Module):
    ...
    def _sample_negative(self, Q, index):
        """ self.teacher_pixel_queue: self.num_classes, self.pixel_memory_size, self.dim"""
        ...
        # now X_ is a tensor with each class sampled with contrast_size, and y_ is the class index
        return X_, y_
    ...
    def _dequeue_and_enqueue(self, features, labels):
        ...
        for bs in range(batch_size):
            this_feat = features[bs].contiguous().view(feat_dim, -1)
            ...
            for lb in this_label_ids:
                ...
                # if reach the limit of a queue, renew the first ptr, there is a que as
                # [label, self.region_memory_size, feat]
