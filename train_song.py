...
def parse_args():
    ...
    parser.add_argument("--lambda-dkd", type=float, default=0.15, help="lambda_dkd")
    parser.add_argument("--lambda-ckd", type=float, default=0.1, help="lambda ckd")
    # parser.add_argument("--lambda-memory-pixel", type=float, default=0.1, help="lambda memory-based pixel")
    parser.add_argument("--lambda-ipm", type=float, default=0.000003, help="lambda img_pair_feature")
    ...
    parser.add_argument('--save-per-iters', type=int, default=500,
                        help='per iters to save')
    parser.add_argument('--val-per-iters', type=int, default=500,
    ...
    time_now = dt.now()
    args.log_dir = os.path.join(args.log_dir, time_now.strftime('%Y-%m-%d_%H-%M-%S'))
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'citys':
            ...
            if self.args.student_model == 'psp':
                self.args.lambda_ckd = 0.05
                self.lambda_task = 1.5
            elif self.args.student_backbone == 'mobilenetv2':
                self.args.lambda_ckd = 0.05
                self.args.lambda_dkd = 0.15
                self.lambda_task = 1.5
            else:
                self.lambda_task = 1

            # self.args.lambda_ipm = 1e-10
            # self.args.max_iterations = 30000
            ...
            self.args.max_iterations = 60000
            self.args.lambda_ckd = 0.04
            self.dkd_set = False
        ...
        # self.crt_pair_img_feature = IPMLoss(student_channels=s_channels, teacher_channels=t_channels,
        #                                     dataset=args.dataset).to(self.device)
        self.criterion_dkd = CriterionDKD(self.dkd_set, args.batch_size, temperature=args.kd_temperature).to(
            self.device)
        self.criterion_ckd = CriterionCKD(temperature=args.kd_temperature,
                                          s_channels=s_channels,
                                          t_channels=t_channels,
                                          ignore_label=args.ignore_label,
                                          feat_size=t_y[0].shape[2:]).to(self.device)
        params_list = nn.ModuleList([])
        params_list.append(self.s_model)
        params_list.append(self.criterion_ckd)
    ...
    def train(self):
        ...
        # 显存分析
        if allocate:
            initial_usage = torch.cuda.memory_allocated() / 1000000
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            if allocate:
                after_input = torch.cuda.memory_allocated() / 1000000
                print("after_input", after_input - initial_usage, after_input)
            with torch.no_grad():
                t_outputs = self.t_model(images)  # [bs, class, h_after, w_after]

            if allocate:
                after_teacher = torch.cuda.memory_allocated() / 1000000
                print("after_teacher", after_teacher - after_input, after_teacher)
            s_outputs = self.s_model(images)

            if allocate:
                after_student = torch.cuda.memory_allocated() / 1000000
                print("after_student", after_student - after_teacher, after_student)
            if self.args.aux:
            ...
            dkd_loss = self.args.lambda_dkd * self.criterion_dkd(s_outputs[0], t_outputs[0], targets)

            ckd_loss = \
                self.args.lambda_ckd * self.criterion_ckd(s_outputs[-1], t_outputs[-1])

            # ipm_loss = self.args.lambda_ipm * self.crt_pair_img_feature(s_outputs[-1], t_outputs[-1], targets)

            # losses = task_loss + dkd_loss + ckd_loss + ipm_loss
            losses = task_loss * self.lambda_task + dkd_loss + ckd_loss
            ...
            kd_losses_reduced = self.reduce_mean_tensor(dkd_loss)
            ckd_loss_reduced = self.reduce_mean_tensor(ckd_loss)
            # ipm_loss_reduced = self.reduce_mean_tensor(ipm_loss)
